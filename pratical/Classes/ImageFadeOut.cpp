...
void ImageFadeOut::update(float d)
{
    ...
    Vec2 pA(
            ...
            _pointA>_contentSize.width?_contentSize.height-(_pointA-_contentSize.width):_contentSize.height
    );
    
    Vec2 pB(
            _pointB>_contentSize.height?_contentSize.width-(_pointB-_contentSize.height):_contentSize.width,
    ...
    while (x<pB.x&&y>0) {
        ...
        auto colorCode=pixel+(static_cast<unsigned int>(_contentSize.height-y-1)*_image->getWidth())+x;
